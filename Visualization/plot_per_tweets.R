...
plot_per_tweets <- function(file_string){
  ...
  argument_frame = data.frame( match = c("economy", "religion", "politics", "tourism", "overall"),
                               files = c("csv/economy_tweets.csv", "csv/religion_tweets.csv", "csv/politic_tweets.csv", "csv/tourism_tweets.csv", "overall_plots")
  )
  ...
  if(file_string == "overall"){
    eco <- read.csv(file="csv/economy_tweets.csv",head=TRUE,sep=",")
    rel <- read.csv(file="csv/religion_tweets.csv",head=TRUE,sep=",")
    pol <- read.csv(file="csv/politic_tweets.csv",head=TRUE,sep=",")
    tour <- read.csv(file="csv/tourism_tweets.csv",head=TRUE,sep=",")
  ...
  leaflet(mapStates)%>% addTiles()%>%
    ...
    addCircleMarkers(lng = df$lng, lat = df$lat,  
                     radius = 1, color = c('red')) %>% setView(0.0,0.0, zoom =2)
